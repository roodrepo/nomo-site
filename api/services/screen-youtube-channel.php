...
if($videoInfo != null){
    $messageTelegram = $videoInfo['title'] . "\n"
        . implode(", ", $videoInfo['tags_video']) . "\n"
        . "https://www.youtube.com/watch?v=" . $videoInfo['videoId']  . "\n\n"
        . "http://nomobot.net/interface/trigger-signal.php?quotes_content=" . implode(",", $videoInfo['quotes']) . "\n\n";
    ...
    if(count($videoInfo['quotes']) > 0) {
        $callmebot = json_decode(file_get_contents('http://api.callmebot.com/start.php?user=@Roods13&text=' . implode(',', $videoInfo['quotes']) . '&lang=en&rpt=4'), true);
    }
}
...
//    . "http://nomobot.net/interface/trigger-signal.php?quotes_content=" . implode(",", $tags_currency_video) . "\n\n";
//
//    $sendTelegram = json_decode(file_get_contents('https://api.telegram.org/' . $ConfigSecret->getSetting('TELEGRAM_API_SECRET') . '/sendMessage?chat_id=' . $ConfigSecret->getSetting('TELEGRAM_CHATID') . '&text=' . urlencode($messageTelegram)), true);
//
